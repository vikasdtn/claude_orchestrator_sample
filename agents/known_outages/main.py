"""FastAPI server for Known Outages Agent."""
...
agent: KnownOutagesAgent | None = None


@asynccontextmanager
async def lifespan(app: FastAPI):
    global agent
    agent = KnownOutagesAgent()
    yield
    if agent:
        agent.gateway.close()


app = FastAPI(title="Known Outages Agent", version="1.0.0", lifespan=lifespan)
