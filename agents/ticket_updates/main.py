"""FastAPI server for Ticket Updates Agent."""
...
agent: TicketUpdatesAgent | None = None


@asynccontextmanager
async def lifespan(app: FastAPI):
    global agent
    agent = TicketUpdatesAgent()
    yield
    if agent:
        agent.gateway.close()


app = FastAPI(title="Ticket Updates Agent", version="1.0.0", lifespan=lifespan)
